...
[Prompt/Intent] ──> [MCP Server] ──> [Dynamic CSV Generation]
│
┌─────────────────── [Self-Healing Loop] ◄──┤
│                                           ▼
▼                                   [Pytest Execution]
[Fix Script] ──> [Analyze Allure Results]     │
▲                   ▼
└───── (If Fail) ── [Exit Code?] ── (If Pass 0 Errors) ──> [Auto Git Push]
...
1. The AI agent intercepts the Pytest exit code and parses raw **Allure results** and traceback logs.
2. It autonomously diagnoses the failure, accesses local system files via MCP tools, and rewrites the broken Playwright scripts.
3. The suite re-executes recursively until it achieves a **zero-error, 100% pass rate gate**.
